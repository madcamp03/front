...
def show_player_detail(basic_info, initial_df, player_id, player_stats):
    player_basic_info = basic_info[player_id]
    name, birth, height, weight, school, position, batting = player_basic_info

    with st.expander(f"{name} 상세 기록 보기"):
        col1, col2 = st.columns([1, 2])

        with col1:
            st.markdown(f"**{name} 기본 정보**")
            basic_info_df = pd.DataFrame([{
                "이름": name, "생년월일": birth, "신장": height, "체중": weight,
                "경력": school, "포지션": position, "투타": batting
            }])
    ...
    with tabs[0]:
        ...
        if search_name:
            ...
            if not results.empty:
                ...
                if selected_name:
                    player_id = hitter_initial_df.loc[hitter_initial_df['Name'] == selected_name, 'player_id'].values[0]
                    show_player_detail(hitter_basic_info, hitter_initial_df, player_id, player_stats)
            else:
            ...
            if not results.empty:
                ...
                if selected_name:
                    if st.button('선수 상세 기록 보기'):
                        player_id = pitcher_initial_df.loc[pitcher_initial_df['Name'] == selected_name, 'player_id'].values[0]
                        # show_player_detail(pitcher_initial_df, player_id)
            else:
                st.write('일치하는 선수가 없습니다.')
